  ...
  return (
    <div className="sidebar  md:w-[15%] h-full md:h-[85vh] m-5 rounded-md border-1 pt-3 pb-11 flex flex-wrap justify-center">
      ...
      <ul className="border-slate-500 mt-3 w-full md:w-[90%] rounded-md h-[85%]">
        {zadaci.length == 0 && <h1 className="text-center uppercase text-white">No tasks!</h1>}
        {zadaci.map(zadatak=>{
        return(<li
         
         className={zadatak.zadatakID === odabraniZad.zadatakID ? "mt-3 flex bg-[#4944d6] text-white p-2 rounded-md hover:opacity-70 relative border-2 border-white" : "mt-3 flex bg-white p-2 rounded-md hover:opacity-70 relative"}
         onClick={()=>{
